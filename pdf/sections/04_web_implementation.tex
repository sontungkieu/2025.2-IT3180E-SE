...
được tinh chỉnh với thanh trên thường, session chip, menu dropdown custom có
trạng thái option rõ, trạng thái focus dễ nhận biết và bố cục mobile tự trở về
đầu trang khi chuyển màn hình làm việc. Thanh trên không sticky để nội dung dài
không trượt bên dưới một lớp header nổi trong lúc vận hành. Giao diện cũng hỗ trợ ba chế độ \textit{Sáng}, \textit{Tối} và
...
trình bày chọn phiên khách đang chờ nhận xe hoặc đang thuê xe, chọn bãi đích,
kéo thả marker GPS người dùng hoặc bấm snap gần bãi, hệ thống sẽ đưa marker về
điểm gần nhất trên tuyến đường demo nội bộ và hiển thị polyline đường đi tới bãi
nhận/trả. Route này không cho kéo xe rảnh không gắn với khách nào; khi chưa có
lượt active ở mode trả xe, UI hiển thị empty state và khóa nút snap thay vì thao
tác trên marker ẩn. Cùng màn này có đồng hồ hệ thống nội bộ:
...
danh sách bãi và phiên GPS sẵn sàng, giao diện hiển thị skeleton loading để không xuất hiện
...
UC002 & Danh sách bãi theo GPS demo/vị trí đã lưu/vị trí nhập tay, lọc loại xe, chọn xe, hủy request & Hệ thống hiển thị bãi trong phạm vi, số xe rảnh, trạng thái từng xe, tạo yêu cầu thuê với thời lượng 1/2/3 giờ khi tọa độ khách nằm trong bán kính bãi và cho hủy request trước khi nhận xe. \\
UC003 & Bảng yêu cầu nhận xe cho Staff/Admin & Nhân sự được scope theo bãi, đối chiếu loại/số giấy tờ, nhận cọc 200k, ghi nhận giấy tờ giữ tại bãi, đổi sang xe rảnh cùng bãi khi cần, kiểm tra lại geofence khách ở bước handover, hủy yêu cầu không đủ điều kiện, tạo rental và chuyển xe sang trạng thái đang thuê. \\
UC004 & Pipeline trả xe, bảng xe đang thuê và thao tác trả xe & Staff nhận xe tại bãi được phân công; Admin/Operator xử lý toàn hệ thống và hỗ trợ trả khác bãi. Customer phải xác nhận bãi trả bằng GPS trong bán kính bãi trước khi staff xuất vé; hệ thống tính phí thuê, chỉ giảm giá cư dân khi tài khoản là resident và thẻ cư dân verified, phụ thu trễ và hiển thị phiếu vừa xuất. \\
...
Backend tests & \texttt{test/app.test.js} & Test đăng ký, availability, resident discount guard, geofence pickup/handover/return, handover, return ticket, reports và concurrent sessions. \\
...
UC002 & Customer rental UI, station/bike APIs & Tìm bãi theo tọa độ hoặc vị trí đã lưu, chọn range/type, hiển thị held bike và tạo request pending pickup sau khi email/identity hợp lệ và vị trí khách nằm trong bán kính bãi. \\
UC003 & Pending request table, \texttt{handoverBike}, \texttt{exchangeRentalRequestBike} & Staff/Admin xác nhận giấy tờ, cọc 200k, scope request theo bãi nhân sự, kiểm tra lại tọa độ pickup trước handover, đổi xe trước handover, tạo rental/deposit và đổi xe sang rented trong transaction. \\
UC004 & Return pipeline, \texttt{confirmRentalReturn}, \texttt{returnBike}, pricing module & Customer xác nhận bãi trả trong bán kính bãi, staff nhận xe khác bãi khi đúng scope, tính ticket với resident discount guard tại thời điểm xuất vé, trả cọc, cập nhật bike station/status và ghi status log. \\
...
POST & \url{/api/rental-requests} & Customer & Tạo yêu cầu thuê xe pending pickup sau khi kiểm tra vị trí khách trong bán kính bãi nhận. \\
...
GET & \url{/api/gps-demo/sessions} & Public demo & Lấy các phiên pending/active để \texttt{/gd} kéo GPS người dùng theo đúng khách đang diễn. \\
GET & \url{/api/staff/pending-requests} & Staff/Admin & Danh sách request chờ nhận xe. \\
POST & \url{/api/staff/handover} & Staff/Admin & Chuyển request thành rental sau khi xác nhận giấy tờ/cọc và geofence pickup. \\
...
UC smoke tests & GPS context riêng, 2 customer + 2 admin sessions, request/cancel/request again, staff handover, customer confirm return, return ticket, chart dashboard, audit log và CSV report export. & \texttt{npm run smoke:uc} \\

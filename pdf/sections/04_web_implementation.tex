...
được tinh chỉnh với thanh trên cố định, session chip, menu dropdown custom có
trạng thái option rõ, trạng thái focus dễ nhận biết và bố cục mobile tự trở về
đầu trang khi chuyển màn hình làm việc. Trên mobile, giao diện bỏ topbar riêng,
đưa scene 3D lên đầu hero, nén các metric thành một hàng ngang và rút rail điều
hướng tối thành dock icon-only nổi ở đáy màn hình, bao gồm cả refresh/logout để
không cần một thanh topbar thứ hai. Ở desktop rộng, các panel có bảng dài như
\textit{Thống kê}, \textit{Yêu cầu nhận xe}, \textit{Danh sách trả xe} và
\textit{Đội xe} chiếm toàn bộ bề ngang workspace để chart và bảng không bị cắt.
Riêng bộ lọc báo cáo không dùng dropdown native của trình duyệt mà dùng cùng
component custom có icon, mô tả và trạng thái chọn; chart tổng thu/phụ thu dùng
cùng thang tiền với chart doanh thu để so sánh tuần/tháng không bị lệch nghĩa.
...
này được đóng thành một dải preview cân với cột nội dung ở màn hình đăng nhập và
chỉ đóng vai trò nhấn thị giác, không thay thế các bảng, form và trạng thái nghiệp
vụ chính.
...
\begin{figure}[p]
\centering
\includegraphics[height=0.82\textheight,keepaspectratio]{customer_mobile.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.96\textwidth,height=0.54\textheight,keepaspectratio]{operations_desktop.png}
...
\caption{Route \texttt{/gd}: điều khiển GPS xe và đồng hồ hệ thống cho demo UC002/UC004}
\end{figure}
...
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
  screen/.style={draw=EcoparkGreen,thick,rounded corners=2pt,fill=SoftGreen,minimum width=3.2cm,minimum height=0.9cm,align=center,font=\small},
  ops/.style={draw=EcoparkAmber,thick,rounded corners=2pt,fill=yellow!10,minimum width=3.2cm,minimum height=0.9cm,align=center,font=\small},
  aid/.style={draw=EcoparkTeal,thick,rounded corners=2pt,fill=cyan!8,minimum width=3.2cm,minimum height=0.9cm,align=center,font=\small},
  arr/.style={-{Latex[length=2.2mm]},draw=Ink,thick}
]
\node[screen] (auth) {Login / Register};
\node[screen,right=1.0cm of auth] (rent) {Customer\\Thuê xe};
\node[screen,right=1.0cm of rent] (history) {Lượt thuê\\Tài khoản};
\node[ops,below=1.0cm of rent] (pickup) {Operations\\Nhận xe};
\node[ops,right=1.0cm of pickup] (return) {Trả xe\\Xuất vé};
\node[ops,right=1.0cm of return] (reports) {Báo cáo\\Chart + CSV};
\node[ops,below=1.0cm of return] (fleet) {Đội xe\\Audit log};
\node[aid,below=1.0cm of auth] (gps) {\texttt{/gd}\\Demo director};

\draw[arr] (auth) -- node[above,font=\scriptsize] {customer} (rent);
\draw[arr] (rent) -- node[above,font=\scriptsize] {request/history} (history);
\draw[arr] (auth) |- node[near start,left,font=\scriptsize] {staff/admin} (pickup);
\draw[arr] (pickup) -- (return);
\draw[arr] (return) -- (reports);
\draw[arr] (return) -- (fleet);
\draw[arr] (gps) -- node[below,font=\scriptsize] {supports UC002/UC004 demo} (rent);
\draw[arr] (gps) |- (return);
\end{tikzpicture}
}
...
Form đăng ký khách có gợi ý nhập liệu cho họ tên, số điện thoại, CCCD/CMND và địa
chỉ. API đăng ký kiểm tra họ tên không chứa số/ký tự lạ, số điện thoại Việt Nam
...
Màn hình vận hành có pipeline trả xe riêng, luôn hiển thị bốn bước: nhận xe, chọn
bãi trả, kiểm tra tình trạng xe và xuất vé. Bảng chi tiết bên dưới vẫn xử lý các
input nghiệp vụ đầy đủ như bãi trả, giờ trả, tình trạng xe và ghi chú kiểm tra,
nhưng pipeline giúp actor thấy rõ luồng UC004 ngay cả khi chưa có rental active.
...
trình bày chọn xe/người thuê và bãi đích, kéo thả marker GPS của xe hoặc bấm snap
gần bãi, hệ thống sẽ đưa xe về điểm gần nhất trên tuyến đường demo nội bộ và hiển
thị polyline đường đi tới bãi nhận/trả. Cùng màn này có đồng hồ hệ thống nội bộ:
...
theo cùng mốc giờ mà customer và staff nhìn thấy.
...
UC002 & Danh sách bãi theo GPS demo/vị trí đã lưu/vị trí nhập tay, lọc loại xe, chọn xe, hủy request & Hệ thống hiển thị bãi trong phạm vi, số xe rảnh, trạng thái từng xe, tạo yêu cầu thuê với thời lượng 1/2/3 giờ và cho hủy request trước khi nhận xe. \\
UC003 & Bảng yêu cầu nhận xe cho Staff/Admin & Nhân sự được scope theo bãi, đối chiếu loại/số giấy tờ, nhận cọc 200k, ghi nhận giấy tờ giữ tại bãi, đổi sang xe rảnh cùng bãi khi cần, hủy yêu cầu không đủ điều kiện, tạo rental và chuyển xe sang trạng thái đang thuê. \\
UC004 & Pipeline trả xe, bảng xe đang thuê và thao tác trả xe & Staff nhận xe tại bãi được phân công; Admin/Operator xử lý toàn hệ thống và hỗ trợ trả khác bãi. Luồng trả xe chọn thời điểm trả, ghi chú tình trạng xe, cập nhật vị trí xe, tính phí thuê, giảm giá cư dân, phụ thu trễ và hiển thị phiếu vừa xuất. \\
...
\midrule
Frontend shell & \texttt{public/app.js}, \texttt{public/styles.css} & Render auth, customer, operations và \texttt{/gd}; quản lý state client, custom dropdown, map, table, form, report charts, audit timeline, demo clock và CSV download. \\
3D scene & \texttt{public/scene.js} & Mount scene Three.js low-poly, tự cleanup scene cũ, giữ \texttt{preserveDrawingBuffer} cho smoke test pixel. \\
...
Pricing & \texttt{server/pricing.js} & Tính phí cơ bản, phụ thu trễ và giảm giá cư dân theo policy đề bài. \\
Backend tests & \texttt{test/app.test.js} & Test đăng ký, availability, handover, return ticket, reports và concurrent sessions. \\
Browser smoke tests & \texttt{scripts/ui-smoke.js}, \texttt{scripts/uc-flow-smoke.js} & Kiểm tra render UI, map/scene, overflow, console error và pipeline UC bằng Playwright. \\
\bottomrule
\end{longtable}
...
UC002 & Customer rental UI, station/bike APIs & Tìm bãi theo tọa độ hoặc vị trí đã lưu, chọn range/type, hiển thị held bike và tạo request pending pickup sau khi email/identity hợp lệ. \\
UC003 & Pending request table, \texttt{handoverBike}, \texttt{exchangeRentalRequestBike} & Staff/Admin xác nhận giấy tờ, cọc 200k, scope request theo bãi nhân sự, đổi xe trước handover, tạo rental/deposit và đổi xe sang rented trong transaction. \\
UC004 & Return pipeline, \texttt{returnBike}, pricing module & Nhận xe khác bãi, tính ticket, trả cọc, cập nhật bike station/status và ghi status log. \\
...
POST & \url{/api/rental-requests} & Customer & Tạo yêu cầu thuê xe pending pickup. \\
...
POST & \url{/api/staff/handover} & Staff/Admin & Chuyển request thành rental sau khi xác nhận giấy tờ/cọc. \\
GET & \url{/api/staff/active-rentals} & Staff/Admin & Danh sách rental đang chạy để xử lý trả xe. \\
POST & \url{/api/staff/return} & Staff/Admin & Trả xe, tính ticket, cập nhật vị trí/trạng thái xe. \\
...
khách hàng, một phiên admin/operator và một tab \texttt{/gd} để kéo GPS của xe
đang diễn và tua giờ hệ thống. Bộ smoke test vẫn kiểm tra mức tải demo tối đa gồm hai khách hàng,
...
\begin{itemize}
  ...
  \item \texttt{/gd}: mở demo director để kéo thả vị trí xe theo tuyến đường demo và tua đồng hồ hệ thống.
...
\midrule
Backend API tests & Đăng ký, validation input, password policy, nâng cấp hash SHA-256 cũ sang PBKDF2, email verification, reset password, duplicate email/phone/CCCD, blocked identity, resident pending, held-bike availability, cancel request, exchange bike, staff station scope, handover, capacity guard, return pricing, reports, audit logs, concurrent sessions. & \texttt{npm test} \\
UI smoke tests & Auth/customer/admin desktop-tablet-mobile render, GSAP load, Three.js canvas pixel, Leaflet marker, custom report dropdown, scale tiền của chart báo cáo, staff wide table/chart không scroll ngang, bảng vận hành hẹp dạng row-card, console error và horizontal overflow. & \texttt{npm run smoke:ui} \\
UC smoke tests & GPS context riêng, 2 customer + 2 admin sessions, request/cancel/request again, staff handover, return ticket, chart dashboard, audit log và CSV report export. & \texttt{npm run smoke:uc} \\
Manual visual review & Screenshot auth, customer rental flow, operations workspace, mobile dock và GPS route. & Ảnh trong \texttt{temp/} và report UI section. \\
\bottomrule
\end{longtable}
...
Smoke test UC mở demo director \texttt{/gd} ở context riêng, kiểm tra marker xe và trạng
...
\begin{itemize}
  ...
  \item Route \texttt{/gd} là công cụ thuyết trình để diễn vị trí xe theo đường
  và tua giờ nội bộ, không phải tracking GPS thật của thiết bị gắn trên xe.
\end{itemize}
